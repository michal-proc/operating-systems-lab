...
#define ALIVE_INTERVAL 10 // Interval in seconds to check client aliveness
...
void handle_client(client_t *client) {
    ...
    while (1) {
        ...
        printf("Received message from %s: %s", client->id, buffer);
...
int main(int argc, char const *argv[]) {
    ...
    while (1) {
        ...
        new_client->alive = 1; // Initialize alive status
        recv(new_socket, new_client->id, BUFFER_SIZE, 0); // Assume first message is the client ID
